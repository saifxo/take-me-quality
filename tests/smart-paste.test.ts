...
const TAB_ROWS = [
  "Queue\t561 / 1465 - Demo Zoe Fields - BIRPK - BE\t07700900123\t2026-08-31 02:12:07\t00:01:25\t» Play « » Download « » Email «",
  "Queue\t557 / 1465 - Demo Zoe Fields - BIRPK - BE\t07700900456\t2026-08-31 02:28:43\t00:02:46\t» Play « » Download « » Email «",
  "Exten\tDemo Eli Brook - BIR - BE\tDemo Zoe Fields - BIRPK - BE\t2026-08-31 02:57:57\t00:00:10\t» Play « » Download « » Email «",
].join("\n");
...
describe("smart paste — portal rows", () => {
  ...
  it("recognises internal extension calls and keeps the other party", () => {
    ...
    expect(c.agentName).toBe("Demo Eli Brook");
    expect(c.siteCode).toBe("BIR");
    expect(c.internalParty).toBe("Demo Zoe Fields - BIRPK - BE");
    expect(c.callerNumber).toBeNull();
  });

  it("handles rows pasted with spaces instead of tabs", () => {
    const r = ok(parsePortalText("Queue 551 / 1465 - Demo Ava Stone - SOL - DE 07799845791 2026-08-31 02:51:45 00:00:40")[0]);
    expect(r.agentName).toBe("Demo Ava Stone");
  ...
  it("handles each cell on its own line (some browsers copy tables this way)", () => {
    const cells = "Queue\n561 / 1465 - Demo Ava Stone - SOL - DE\n07700900123\n2026-08-31 02:12:07\n00:01:25\n» Play «\n» Download «\n» Email «";
    const rows = parsePortalText(cells);
    expect(rows).toHaveLength(1);
    expect(ok(rows[0]).agentName).toBe("Demo Ava Stone");
  });
  ...
  it("keeps paste order when strict and fallback rows are mixed", () => {
    const text = [
      "Queue 561 / 1465 - Demo Ava Stone - SOL - DE 07700900123 2026-08-31 02:12:07 00:01:25",
      "Queue 561 / 1470 - Demo Max Hill 07700900999 2026-08-31 03:00:00 00:02:00",
      "Queue 561 / 1465 - Demo Ava Stone - SOL - DE 07700900456 2026-08-31 04:12:07 00:01:25",
    ].join("\n");
    const rows = parsePortalText(text).map(ok);
    expect(rows.map((r) => r.callAt)).toEqual([...rows.map((r) => r.callAt)].sort());
    expect(rows[1]).toMatchObject({ agentName: "Demo Max Hill", siteCode: null, teamCode: null });
  });

  it("accepts withheld callers, +44 numbers and UK-format dates", () => {
    const withheld = ok(parsePortalText("Queue 561 / 1465 - Demo Ava Stone - SOL - DE Anonymous 2026-08-31 02:12:07 00:01:25")[0]);
    expect(withheld.anonymousCaller).toBe(true);
    expect(withheld.callerMasked).toBe("Withheld");

    const intl = ok(parsePortalText("Queue 561 / 1465 - Demo Ava Stone - SOL - DE +447700900123 31/08/2026 14:05 01:25")[0]);
  ...
  it("uses GMT outside British Summer Time", () => {
    const r = ok(parsePortalText("Queue 561 / 1465 - Demo Ava Stone - SOL - DE 07700900123 2026-12-01 09:00:00 00:01:00")[0]);
  ...
  it("returns friendly errors instead of throwing on junk", () => {
    const rows = parsePortalText("hello there 123\nQueue 561 / 1465 - Demo Ava Stone - SOL - DE 07700900123 2026-02-31 02:12:07 00:01:25");
  ...
  it("builds forgiving roster keys", () => {
    expect(nameKey("  Demo  Eli Brook ")).toBe(nameKey("demo eli brook"));
